...
public interface ArticleMapper extends CommonMapper<Article> {
  ...
  @Select(
      " SELECT category.id,category.category_name,COUNT(article.id) count from article LEFT JOIN category ON article.category_id=category.id GROUP BY category_id")
  List<StatisticsArticleCategoryResponse> statisticsArticleCategory();

  @Select(
      "SELECT date_format(update_time, '%Y年%m月') AS updateTime, COUNT(id) count FROM article GROUP BY date_format(update_time, '%Y年%m月') ORDER BY update_time DESC")
  List<StatisticsArticleByDateResponse> statisticsArticleByDate();

  @Select(
      "SELECT id as article_id,title,show_count from article WHERE show_count !=0 ORDER BY show_count DESC LIMIT 5")
